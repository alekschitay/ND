#!/usr/bin/env python3
"""
Умный бот - уведомления ТОЛЬКО при реально новых событиях
"""
...
📱 Уведомления будут приходить в этот чат
    ...
    def is_valid_event(self, event: dict) -> bool:
        ...
        # Фильтруем пустые или слишком короткие заголовки
        if len(title) < 5:
            return False
        
        # Фильтруем служебные элементы
        invalid_keywords = [
            'реклама', 'advertisement', 'ad', 'баннер', 'banner',
            'навигация', 'navigation', 'меню', 'menu', 'footer',
            'header', 'sidebar', 'widget', 'виджет', 'cookie',
            'политика', 'policy', 'условия', 'terms', 'контакты',
            'contacts', 'о нас', 'about', 'поддержка', 'support',
            'войти', 'login', 'регистрация', 'register', 'поиск',
            'search', 'фильтр', 'filter', 'сортировка', 'sort',
            'загрузить', 'load', 'показать', 'show', 'скрыть',
            'hide', 'далее', 'next', 'предыдущая', 'previous',
            'страница', 'page', 'следующая', 'следующий'
        ]
        ...
        try:
            ...
            for selector in selectors:
                ...
                if elements:
                    ...
                    for element in elements[:15]:  # Ограничиваем количество
                        try:
                            # Извлекаем заголовок
                            title = ""
                            title_selectors = ['h1', 'h2', 'h3', 'h4', 'h5', '.title', '.name', 'a', '.event-title', '.item-title']
                            ...
                            # Извлекаем дату
                            date = ""
                            date_selectors = ['.date', '.time', '[class*="date"]', '[class*="time"]', '.event-date', '.item-date']
            ...
            event_keywords = [
                ...
                'arena', 'театр', 'theater', 'театр', 'theatre', 'цирк',
                'circus', 'выставка', 'exhibition', 'мероприятие', 'event',
                'билет', 'ticket', 'продажа', 'sale', 'афиша', 'poster'
            ]
            ...
            logger.info(f"Отфильтровано событий: {len(filtered_events)} из {len(events)}")
    ...
    def run(self):
        ...
        logger.info("🎵 Запуск умного бота...")
